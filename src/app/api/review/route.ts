    ...
    const runAnalysis = async () => {
      ...
      try {
        ...
        if ('code' in result) {
          ...
          if (!claimed) console.log(`[review] session ${sessionId} — terminal write skipped, status already settled`)
          return
        }
        ...
        if (!claimed) {
          console.log(`[review] session ${sessionId} — terminal write skipped, status already settled`)
        } else {
