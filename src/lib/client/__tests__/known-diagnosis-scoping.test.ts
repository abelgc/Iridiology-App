...
const mockReport: ReportContent = {
  ...
  section_6_circulatory_cardiorespiratory: 'Cardiac rhythm is stable.',
...
function plannerFixtureWith(knownDiagnoses: Array<{ condition: string; assignedSection: string }>) {
  return {
...
function implWith(knownDiagnoses: Array<{ condition: string; assignedSection: string }>) {
  return (params: any) => {
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify(
              writerFixture(['section_11_detected_axes', 'section_12_conclusion', 'section_13_strengths_of_the_body']),
            ),
          },
        ],
      })
...
describe('known-diagnosis section scoping (REGRESSION: Bhargavi Dasi over-anchoring case, 2026-08-24)', () => {
  it('does not leak a diagnosis assigned to one section into a Writer group that does not own that section', async () => {
    createMock.mockImplementation(
      implWith([{ condition: 'hyperparathyroidism / raised PTH', assignedSection: 'section_5_endocrine_hormonal' }]),
    )
    ...
    const writerC = createMock.mock.calls.find(([p]: any) => p.system.includes('You are Writer C'))

    expect(writerB![0].messages[0].content).not.toContain('hyperparathyroidism')
    expect(writerC![0].messages[0].content).not.toContain('hyperparathyroidism')
  })

  it('does send it to the Writer group that owns the assigned section', async () => {
    createMock.mockImplementation(
      implWith([{ condition: 'hyperparathyroidism / raised PTH', assignedSection: 'section_5_endocrine_hormonal' }]),
    )
    await rewriteReportForClient(mockReport, 'en', 'Jane')
    ...
    expect(writerA![0].messages[0].content).toContain('hyperparathyroidism')
  })

  it('scopes multiple diagnoses to their own assigned writer groups independently', async () => {
    createMock.mockImplementation(
      implWith([
        { condition: 'hyperparathyroidism / raised PTH', assignedSection: 'section_5_endocrine_hormonal' },
        { condition: 'sciatica', assignedSection: 'section_10_structural_integumentary' },
      ]),
    )
    await rewriteReportForClient(mockReport, 'en', 'Jane')
    ...
    expect(writerA[0].messages[0].content).toContain('hyperparathyroidism')
    expect(writerA[0].messages[0].content).not.toContain('sciatica')
    expect(writerB[0].messages[0].content).toContain('sciatica')
    expect(writerB[0].messages[0].content).not.toContain('hyperparathyroidism')
    expect(writerC[0].messages[0].content).not.toContain('hyperparathyroidism')
    expect(writerC[0].messages[0].content).not.toContain('sciatica')
  })

  it('handles an empty knownDiagnoses list exactly as before (no scoping side effects)', async () => {
