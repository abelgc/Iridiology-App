...
DIFFICULT EYES (a dense, dark, or murky iris — worst case, not merely a partial-visibility case): do not guess at the cause of the murkiness or darkness itself — never speculate about what specific combination of factors produced it. Instead anchor on coarse landmarks first: confirm the autonomic wreath's shape and symmetry, the stomach zone, and the bowel zone before attempting any fine-grained sign detection — these remain the most reliably extractable features even when detail elsewhere is unavailable. Calibrate darkness expectations to this individual's own baseline rather than a fixed threshold tuned for lighter eyes — a dense brown iris is not automatically pathological, it is a different starting point to read from. Treat a difficult eye as a lower-confidence, more provisional case, not an empty one.
