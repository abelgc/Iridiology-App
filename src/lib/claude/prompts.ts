...
State each specific finding ONCE, in its home section. A finding's home is its classification: improvements live in Areas of Burden Reduction; unresolved or worsening burdens live in Areas Requiring Continued Attention; stable findings in Stable Constitutional Findings; new findings in New Findings. The Major Changes section names only the headline items in brief — it does not carry their full detail. System Interpretation adds ONLY mechanism and system connections; it must never re-describe a change already stated. Clinical Priorities adds ONLY the prioritised action; it must never re-describe a finding. Before finalising, scan the whole report: if the same territory's change is described in more than one section, keep the fullest occurrence in its home section and reduce every other mention to a brief named reference, or delete it. Repeating a finding across sections is a defect, not thoroughness.
...
{
  "comp_1_major_changes": "HEADLINE ONLY, 3 to 5 sentences. Name the single most significant change and the single most important unresolved burden, plus at most one other headline shift. Name each briefly — do NOT describe its full detail here; the detail belongs to its home section below. This is the summary the practitioner reads first, not a catalogue.",
  "comp_2_burden_reduction": "HOME SECTION for every improvement (categories A and B). Describe each improvement in full, once, here: reduced overlay, congestion, compression, hepatic burden, nervous tension, respiratory density, digestive congestion, improved circulatory openness. Name the territory and system for each. State functional improvement clearly even where structure is unchanged. Do not restate these improvements in any later section.",
  "comp_3_stable_constitutional": "What genuinely did not change (category C), stated once and briefly. Do not re-describe the whole iris. Persistence here is expected, not failure.",
  "comp_4_new_findings": "Genuinely new findings (category D) visible now that were not present or noted previously. If there are none, say so in one sentence.",
  "comp_5_continued_attention": "HOME SECTION for every unresolved or worsening burden (category E and persistent dominant burdens). Describe each in full, once, here. Distinguish persistent-but-stable from genuinely worsening. Do not restate these in any later section.",
  "comp_6_system_interpretation": "MECHANISM AND CONNECTIONS ONLY. Do NOT re-describe or re-list the findings already stated above. Reference them by name in a clause and add only what is new: why the changes are happening, how the systems drive one another (the cascade), primary versus secondary, and phase transitions (for example Congested to Clearing, Overloaded to Rebuilding). If a sentence here merely restates a change described above, delete it.",
  "comp_7_clinical_priorities": "ACTION ONLY. State the overall trajectory in one line, then the prioritised next steps. Reference each finding by name and state what to DO about it — not what it is. Do not re-describe findings already covered. Credit improvements without generic reassurance."
}`
