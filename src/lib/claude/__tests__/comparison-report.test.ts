...
describe('comparison evolution schema', () => {
  it('has exactly the 7 evolution keys, none of them system-named', () => {
    ...
    expect(COMPARISON_REPORT_SECTION_KEYS[0]).toBe('comp_1_major_changes')
  ...
  it('rejects a missing section and rejects empty values', () => {
    expect(comparisonReportContentSchema.safeParse({ comp_1_major_changes: 'x' }).success).toBe(false)
