...
- Domain: owner is buying **narasimhasolutions.com** as a dedicated domain (decided against reusing a subdomain of the already-owned narasimhaclay.com, for cleaner branding). Needed before activating Stripe live mode — account activation, Apple Pay domain verification, and ideally a stable custom domain for the webhook (though the existing `iridiology-app-ten.vercel.app` Vercel alias is already stable/permanent and works fine as an interim webhook target).
...
- [ ] 10. Once the domain is bought and pointed: switch Stripe keys from test to live in Production, activate PayPal/Bizum/Google Pay/Apple Pay for real, verify Apple Pay domain association, remove `ENABLE_MOCK_PAYMENT` from Production, merge `staging` → `master`. **Blocked on**: `narasimhasolutions.com` purchased and DNS pointed at Vercel.
